package com.hansuo.portal.test.httpclient;
...
//		第五步：创建一个HttpGet对象，需要制定一个请求的url
		HttpGet get = new HttpGet("http://itheima.com");
//		第六步：执行请求。
...
//		第三步：创建一个list模拟表单，list中每个元素是一个NameValuePair对象
		...
		fromList.add(new BasicNameValuePair("name", "张三"));
		fromList.add(new BasicNameValuePair("pass","1243"));
//		第四步：需要把表单包装到Entity对象中。StringEntity
		StringEntity entity = new UrlEncodedFormEntity(fromList,"utf-8");
